...
janela.set_title("Joguito - Versão Final")
...
spaws = [1317, 29, 1332, 19, 670, 573, 526, 689] # (x,x,x,x, y,y,y,y) simplificado
vidas, inis_ativos, personagens, Frangos = [], [], [], []
tirosD, tirosE = [], []

# LISTA NOVA PARA TIROS DO BOSS
tirosBoss = []
...
soco_ja_hitou = False # NOVA VARIÁVEL: Para limitar a 1 hit por soco
...
def reset():
    ...
    inis_ativos.clear()
    Frangos.clear()
    tirosBoss.clear() # Limpa tiros do boss ao resetar
...
def boss_atira(boss, alvo):
    # Boss usa os mesmos sprites de tiro do Wumberto
    # Atira horizontalmente na direção que está olhando
    if boss["direita"]:
        tiro = Sprite("bala.png")
        tiro.x = boss["x"] + boss["largura"]
        tiro.y = boss["y"] + 40 # Ajuste de altura aproximado
        tiro.vel_x = vel_tiro
    else:
        tiro = Sprite("bala_reverse.png")
        tiro.x = boss["x"]
        tiro.y = boss["y"] + 40
        tiro.vel_x = -vel_tiro
        
    tirosBoss.append(tiro)
...
def chama_ini():
    ...
    tipo = random.choice(tipos_inimigos[:3]) # Garante que não pega o boss (índice 3) no spawn normal
...
def chama_boss():
    ...
    tipo = tipos_inimigos[3] # Sprite do BOSS
...
def gerenciar_inimigos(dt):
    ...
    # Velocidade base aumenta com fase. E TUDO É 15% MAIS RÁPIDO (* 1.15)
    ...
    for i, ini in enumerate(inis_ativos):
        ...
        # --- LÓGICA DE TIRO DO BOSS ---
        if ini.get("eh_boss"):
            ini["timer_tiro"] += dt
            # Atira se: distancia > 400 E cooldown > 5s
            if dist_min > 400 and ini["timer_tiro"] >= 5.0:
                boss_atira(ini, alvo)
                ini["timer_tiro"] = 0.0 # Reseta cooldown
...
while rodando:   
    ...
    # Controle de Piscada/Invencibilidade
    ...
    if state == "comeco":
        ...
        if mouse.is_over_object(botao_jogar) and mouse.is_button_pressed(1) and mouse_livre:
            state, mouse_livre = "jogando", False
        if mouse.is_over_object(botao_opcao) and mouse.is_button_pressed(1) and mouse_livre:
            state, mouse_livre = "opcao", False
        if mouse.is_over_object(botao_sair) and mouse.is_button_pressed(1) and mouse_livre: break
        ...
        if mouse.is_over_object(botao_mult) and mouse.is_button_pressed(1) and mouse_livre:
            ...
            multiplayer, mouse_livre, state = True, False, "jogando"
            vida_F, vida_W = 3, 1000
            
    elif state == "jogando":
        if not passou_por_op: personagens = [wumberto]
        mouse_livre = False
        ...
        if prox_fase <= 0:
            fase += 1
            if fase > 2: # Vitória após fase 2 (Boss morto)
                state = "game_over"
                janela.draw_text("PARABÉNS! VOCÊ VENCEU!", janela.width/2 - 200, janela.height/2, 40, amarelo, "Calibri", True)
        ...
        if flavia in personagens:
            ...
            if teclado.key_pressed("space") and timer >= 0.5:
                timer = 0
                if fla_D: da_tiro_D(flavia)
                else: da_tiro_E(flavia)
        
        # Gerenciar Tiros Jogadores
        ...
        # Gerenciar Tiros do Boss
        for i in range(len(tirosBoss) -1, -1, -1):
            tb = tirosBoss[i]
            tb.draw()
            tb.x += tb.vel_x * delta_time
            
            # Remove se sair da tela
            if tb.x > janela.width or tb.x < -tb.width:
                tirosBoss.pop(i)
                continue
            ...
            for p in personagens:
                if tb.collided(p):
                    if p == flavia: tomar_dano_flavia()
                    elif p == wumberto: tomar_dano_wumberto()
                    hit = True
                    break
            if hit: tirosBoss.pop(i)
        ...
        if wumberto in personagens: 
            ...
            if teclado.key_pressed("K"):
                if not Soco_Ativo: # Inicia o soco
                    Soco_Ativo = True
                    tempo_soco = 0.0
                    soco_ja_hitou = False # Reseta a flag de hit único
                
                # Posiciona o hitbox do soco
            ...
            if teclado.key_pressed("L"):
                ...
                if timerW >= 0.5:
                    timerW = 0
                    if wum_D: da_tiro_D(wumberto)
                    else: da_tiro_E(wumberto)
            else: dando_tiro_w = False
            
        # Lógica de Dano do Soco (Limitado a 1 inimigo por soco)
        if Soco_Ativo:
            tempo_soco += delta_time
            if tempo_soco >= DURACAO_SOCO: Soco_Ativo = False
            elif not soco_ja_hitou: # Só verifica se ainda não bateu em ninguém
                for i in range(len(inis_ativos)-1, -1, -1):
                    if Soco.collided(inis_ativos[i]["sprite_atual"]): 
                        causar_dano_ini(i)
                        soco_ja_hitou = True # Marca que bateu
                        break # Sai do loop para não bater em mais ninguém
        ...
        # Spawn Boss Fase 3 (após matar 3 inimigos iniciais)
        if fase == 2 and prox_fase <= 17 and not boss_spawnado:
            chama_boss()

        # Spawn Normal (acelerado)
        if q_ativos < total_tela:
            intervalo_spawn = 3.0 - (fase * 0.3)
        ...
        for idx in sorted(frangos_remov, reverse=True): Frangos.pop(idx)
        ...
        if teclado.key_pressed("SPACE"): reset()
    
    esc_anterior = esc_atual
    if not mouse.is_button_pressed(1): mouse_livre = True
    janela.update()